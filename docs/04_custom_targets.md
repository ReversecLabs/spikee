...
Spikee v5.5 introduced support for multi-turn attacks, using compatible attack and target scripts, which enable Spikee to assess conversational LLM applications against multi-turn prompt injection attacks.

As part of this the `MultiTarget` parent class has been implemented, which includes built-in support for managing conversation ID and history storage in a multiprocessing safe way. 

**Variables Explained:**
* Spikee Session ID: This is a UUID generated by a Spikee attack script to uniquely identify a multi-turn attack entry.
* Session ID: This is a UUID generated by the target application or the target script to identify an application conversation. 

**Functions Explained:**
* `_get_spikee_session_data(session_id)`: Retrieves stored data for a given Spikee session ID.
* `_update_spikee_session_data(session_id, data)`: Updates stored data for a given Spikee session ID.
* `_get_spikee_id_correlation(spikee_session_id)`: Retrieves a list of session IDs correlated to a Spikee session ID. Last ID in list is most recent.
* `_add_spikee_id_correlation(spikee_session_id, target_id)`: Adds a session ID correlation to a Spikee session ID.

See `LLMBank` target for an example implementation of a `MultiTarget` target.
