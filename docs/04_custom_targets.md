...
* Target Session ID: This is the identifier used by the target application to track the chat session. Its format (e.g., UUID, integer, string) is determined by the specific target implementation.

**Functions Explained:**
* `_get_spikee_session_data(spikee_session_id)`: Retrieves stored data for a given Spikee session ID.
* `_update_spikee_session_data(spikee_session_id, data)`: Updates stored data for a given Spikee session ID.
* `get_target_session_id(spikee_session_id)`: Retrieves the target session ID correlated to a Spikee session ID.
* `set_target_session_id(spikee_session_id, target_session_id)`: Sets/Overwrites the single session ID correlation to a Spikee session ID.
